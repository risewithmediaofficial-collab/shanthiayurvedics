...
export const seedComprehensiveData = async () => {
  try {
    ...
    // 5. Wellness Advisers / Doctors
    const doctorData = {
      name: 'Dr. Shanthi V (BAMS, MD Ayurveda)',
      category: 'AYURVEDIC_DOCTOR',
      specialization: 'Metabolism, Weight Management & Sandhi Vata',
    ...
    const doctor = await Doctor.findOneAndUpdate({ email: doctorData.email }, doctorData, { upsert: true, new: true });
    logger.info('✅ Ayurvedic Doctor profile seeded');
    ...
    const sampleLeadNames = [
      ...
      { name: 'Manjunath Swamy', phone: '9845112201', city: 'Bengaluru', source: LEAD_SOURCES.META, status: LEAD_STATUS.INTERESTED, notes: 'Severe knee pain, wants doctor consult' },
    ...
    // 8. Doctor / Wellness Consultation Bookings (12 Appointments)
    const todayStr = new Date().toISOString().split('T')[0];
    const consultSlots = [
      { time: '10:00 AM', name: 'Venkatesh Raman', phone: '9840112233', concern: 'Joint & Knee Pain', status: 'COMPLETED' },
      { time: '10:30 AM', name: 'Meenakshi Sundaram', phone: '9842155667', concern: 'Weight Loss & Metabolism', status: 'COMPLETED' },
      { time: '11:00 AM', name: 'Priya Dharshini', phone: '9845019871', concern: 'Slim Herbal Dosage Review', status: 'CONFIRMED' },
      { time: '11:30 AM', name: 'Manjunath Swamy', phone: '9845112201', concern: 'Knee Cartilage Care', status: 'CONFIRMED' },
      { time: '12:00 PM', name: 'Saravanan M', phone: '9845112205', concern: 'Detox Tea & Acidity', status: 'CONFIRMED' },
      { time: '02:30 PM', name: 'Balaji K', phone: '9845112213', concern: 'General Ayurvedic Health', status: 'CONFIRMED' },
      { time: '03:00 PM', name: 'Kavitha Natarajan', phone: '9443277889', concern: 'Sandhi Joint Followup', status: 'CONFIRMED' }
    ];

    for (const c of consultSlots) {
      await DoctorSlotBooking.findOneAndUpdate(
        { date: todayStr, timeSlot: c.time },
        {
          doctorId: doctor._id,
          branchId: hosurBranch._id,
          bookedBy: madhuUser._id,
          date: todayStr,
          timeSlot: c.time,
          patientName: c.name,
          patientMobile: c.phone,
          consultationType: 'TELEMEDICINE',
          healthConcern: c.concern,
          status: c.status,
          notes: 'Regular wellness advisory session'
        },
        { upsert: true }
      );
    }
    logger.info(`✅ ${consultSlots.length} Patient Wellness Consultations seeded for today`);

    // 9. Orders across Lifecycle States (25+ Real Orders)
    ...
    return {
      ...
      consultCount: consultSlots.length,
